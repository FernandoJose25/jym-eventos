...
export default function GaleriaPage() {
  ...
  useEffect(() => {
    getDocs(query(collection(db,'gallery_items'), orderBy('order','asc')))
      .then(snap => {
        setItems(snap.docs.map(d => ({ id:d.id, ...d.data() })).filter((i:any) => i.visible !== false) as GItem[]);
        setLoading(false);
      });
  }, []);
  ...
  return (
    <>
      ...
      {/* Filtros + búsqueda */}
      <section style={{ background:'#f8fafc', padding:'1.5rem 0 0', position:'sticky', top:72, zIndex:50,
        ...
        <div className="container">
          ...
          <div style={{ position:'relative', maxWidth:520, marginBottom:'1rem' }}>
            <Search size={16} style={{ position:'absolute', left:14, top:'50%', transform:'translateY(-50%)',
          ...
          {haySubs && (
            <div style={{ display:'flex', gap:6, overflowX:'auto', paddingBottom:'1rem', flexWrap:'wrap' }}>
              <p style={{ fontSize:'0.7rem', fontWeight:700, textTransform:'uppercase', letterSpacing:'.1em',
        ...
        <div className="container">
          {loading ? (
            <div style={{ columns:'3 220px', gap:'1rem' }}>
              {[...Array(12)].map((_,i) => (
                <div key={i} className="skeleton" style={{ breakInside:'avoid', marginBottom:'1rem',
                      ...
                      <div style={{ display:'flex', gap:8, flexWrap:'wrap', justifyContent:'center' }}>
                        {suggestions.map(s => (
                          <button key={s} onClick={() => setSearchQ(s)}
                                  style={{ padding:'0.4rem 1.1rem', borderRadius:9999,
                ...
                {visibles.map((item, i) => {
                  ...
                  return (
                    <div key={item.id}
                         ...
                         style={{ breakInside:'avoid', marginBottom:'1.25rem', borderRadius:16,
                      ...
                      {/* Video badge */}
                      {vid && (
                        <div style={{ position:'absolute', top:10, left:10, background:'rgba(10,22,40,0.82)',
                      ...
                      <div className="gal-overlay"
                        ...
                        <div style={{ position:'absolute', bottom:12, left:14, right:14 }}>
                          {item.categoria && (
                            <span style={{ display:'inline-block', background:'rgba(212,160,23,0.9)',
      ...
      <section style={{ padding:'4rem 0', background:'linear-gradient(135deg,#0a1628,#1e3a5f)', textAlign:'center' }}>
        <div className="container">
          ...
          <div style={{ display:'flex', flexWrap:'wrap', gap:'1rem', justifyContent:'center' }}>
            <a href="/contacto"
               style={{ display:'inline-flex', alignItems:'center', gap:8, padding:'1rem 2rem',
