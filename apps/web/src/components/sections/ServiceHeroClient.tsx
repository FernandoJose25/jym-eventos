...
export default function ServiceHeroClient({ servicio: s }: { servicio: Servicio }) {
  ...
  return (
    <section ref={ref} style={{ minHeight:'65vh', display:'flex', alignItems:'center', position:'relative', overflow:'hidden', background:'linear-gradient(135deg,#050d1a,#0a1628,#1e3a5f)', paddingTop:72 }}>
      ...
      <div style={{ position:'absolute', inset:0, background:'linear-gradient(to right,rgba(5,13,26,.8),rgba(5,13,26,.3))' }} />
      <div className="container" style={{ position:'relative', zIndex:10 }}>
        <nav style={{ color:'rgba(255,255,255,0.45)', fontSize:'0.8rem', marginBottom:16 }}>
        ...
        <h1 style={{ color:'#fff', marginBottom:'1.25rem', maxWidth:700 }} dangerouslySetInnerHTML={{ __html: s.hero?.h1||s.tituloCorto }} />
        {s.hero?.descripcion && <p style={{ color:'rgba(255,255,255,.75)', fontSize:'1.1rem', maxWidth:580, marginBottom:'2rem' }}>{s.hero.descripcion}</p>}
