...
import {
  ...
  ChevronRight, Briefcase, Bell, ChevronDown, Layers,
} from 'lucide-react';
...
const NAV = [
  ...
  { href:'/dashboard/configuracion', icon:Settings,        label:'Configuración', group:'sistema' },
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  if (loading || !user) return (
    <div style={{ minHeight:'100vh', display:'flex', alignItems:'center', justifyContent:'center', background:'#0a1628' }}>
      <div style={{ width:40, height:40, border:'3px solid rgba(212,160,23,0.3)', borderTopColor:'#d4a017',
    ...
    <div style={{ display:'flex', flexDirection:'column', height:'100%' }}>
              ...
              {items.map(({ href, icon:Icon, label:lbl, badge }) => {
                ...
                return (
                  <Link key={href} href={href} onClick={()=>setSideOpen(false)}
                        style={{ display:'flex', alignItems:'center', gap:10, padding:'0.55rem 0.75rem',
                    ...
                    <div style={{ paddingLeft:10, display:'flex', flexDirection:'column', gap:1, marginBottom:4 }}>
                      ...
                      {srvList.map(s => (
                        <Link key={s.id}
                              ...
                              style={{ display:'flex', alignItems:'center', gap:8, padding:'0.4rem 0.75rem',
      ...
      <div style={{ padding:'0.875rem', borderTop:'1px solid rgba(255,255,255,0.08)' }}>
        <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:8 }}>
          <div style={{ width:32, height:32, borderRadius:8, background:'linear-gradient(135deg,#1e3a5f,#2563eb)',
        ...
        <button onClick={signOut}
                style={{ width:'100%', display:'flex', alignItems:'center', gap:8, padding:'0.45rem 0.75rem',
  ...
  return (
    <div style={{ display:'flex', minHeight:'100vh', background:'#f0f4f8' }}>
      ...
      {/* Sidebar desktop */}
      <aside style={{ width:'var(--jym-sidebar-w)', flexShrink:0, background:'#0a1628',
